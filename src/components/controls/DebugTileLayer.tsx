type DebugTileLayerProps = {
	isTileDebugEnabled: boolean;
	onToggleTileDebug: () => void;
};

function DebugTileLayer({
	isTileDebugEnabled,
	onToggleTileDebug,
}: DebugTileLayerProps) {
	return (
		<div className='control'>
			<label htmlFor='tile-debug-toggle'>Tile debug</label>
			<button
				id='tile-debug-toggle'
				type='button'
				className={`control-button ${
					isTileDebugEnabled ? 'is-active' : ''
				}`}
			...
			>
				...
				{isTileDebugEnabled ? 'Hide tile debug' : 'Show tile debug'}
			</button>
		</div>
	);
}
